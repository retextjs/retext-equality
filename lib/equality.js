    ...
    while (++index < length) {
        ...
        for (phrase in inconsiderate) {
            ...
            if (firstWord in byWord) {
                byWord[firstWord].push(index);
            } else {
                byWord[firstWord] = [index];
            }
        }
    }
})();
...
function simple(matches, pattern, file) {
    ...
    while (++index < length) {
        nodes = matches[index].nodes;

        warn(file, toString(
            nodes
        ), pattern.considerate, nodes[0], pattern.note);
    }
}
...
function or(matches, pattern, file) {
    ...
    while (++index < length) {
        ...
        warn(file, value, pattern.considerate, nodes[0], pattern.note);
    }
}
...
function attacher(processor, options) {
    ...
    function transformer(cst, file) {
        visit(cst, T_PARAGRAPH, function (node) {
            ...
            for (key in matches) {
                handlers[byId[key].type](matches[key], byId[key], file);
            }
        });
    }
